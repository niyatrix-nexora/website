...
void main(){
  ...
  gl_FragColor=vec4(col,clamp(alpha,0.0,1.0));
    ...
    const obs = new IntersectionObserver(entries => {
      entries.forEach(e => {
        if (e.isIntersecting) {
          ...
          // Trigger scramble text on eyebrows inside the revealed element (disabled to prevent conflict with typewriter)
          /*
    ...
    // ══════════════════════════════════════════════
    /**
     * Types out plain text node contents character by character.
     * Handles elements with inline HTML (like <br>, <span>, <em>) by
     * preserving the HTML and only animating plain text leaf nodes.
     * Shows a blinking cursor while typing, removes it on completion.
     */
    function typewriterEl(el, opts = {}) {
      if (el.dataset.twDone || el.dataset.twActive) return;
      el.dataset.twActive = 'true';

      const speed = opts.speed || 22;        // ms per character
      const initialDelay = opts.delay || 0;  // ms before starting

      // Collect all text nodes recursively (excluding script/style)
      ...
      // Build a flat queue of { node, char } pairs for sequential typing
      const textNodes = getTextNodes(el);
      if (!textNodes.length) { el.dataset.twDone = 'true'; return; }

      // Store originals and blank them
      const originals = textNodes.map(n => n.textContent);
      textNodes.forEach(n => { n.textContent = ''; });

      // Inject blinking cursor element after the element
      const cursor = document.createElement('span');
      cursor.className = 'tw-cursor';
      cursor.textContent = '|';
      cursor.style.cssText = 'display:inline-block;margin-left:1px;opacity:1;animation:twBlink 0.65s step-end infinite;color:inherit;font-weight:300;';
      el.appendChild(cursor);

      // Build queue: [{nodeIdx, charIdx}]
      const queue = [];
      originals.forEach((text, nodeIdx) => {
        for (let ci = 0; ci < text.length; ci++) {
          queue.push({ nodeIdx, charIdx: ci });
        }
      });
      ...
      function typeNext() {
        if (qIdx >= queue.length) {
          ...
          setTimeout(() => {
            ...
            delete el.dataset.twActive;
          }, 600);
          return;
        }
        const { nodeIdx, charIdx } = queue[qIdx];
        textNodes[nodeIdx].textContent += originals[nodeIdx][charIdx];
        qIdx++;
    ...
    // Expose typewriterEl globally
    window.typewriterEl = typewriterEl;

    // Inject the blink keyframes into the document (once)
    ...
    // Observe ALL text elements across every section (except #hero, nav, footer nav, buttons)
    // A broad selector that catches every meaningful text node outside the hero
    const twSelector = [
      'section:not(#hero) h1',
      'section:not(#hero) h2',
      'section:not(#hero) h3',
      ...
      'section:not(#hero) .usp-title',
      'section:not(#hero) .usp-desc',
      'section:not(#hero) .stat-label',
      'section:not(#hero) .position-title',
      'section:not(#hero) .position-desc',
      'section:not(#hero) .position-type',
      'section:not(#hero) .position-location',
      'section:not(#hero) .position-salary',
      'section:not(#hero) .blog-title',
      'section:not(#hero) .blog-excerpt',
      'section:not(#hero) .blog-category',
      'section:not(#hero) .blog-date',
      'section:not(#hero) .testi-body',
      'section:not(#hero) .font-logo',
      'section:not(#hero) .text-muted2'
    ].join(',');
    ...
    const twObs = new IntersectionObserver(entries => {
      entries.forEach(e => {
        if (e.isIntersecting) {
          const el = e.target;
          twObs.unobserve(el);
          // Speed based on element type: headings slower, body text fast
          const tag = el.tagName;
          let speed = 8;
          if (tag === 'H1') speed = 18;
          else if (tag === 'H2') speed = 16;
          else if (tag === 'H3') speed = 14;
          else if (tag === 'H4') speed = 12;
          else if (tag === 'LI') speed = 6;
          typewriterEl(el, { speed, delay: 150 });
        }
      });
    }, { threshold: 0.15 });

    // Expose twObs globally
    window.twObs = twObs;

    const allTextEls = Array.from(document.querySelectorAll(twSelector));
    
    // Filter out elements that have an ancestor in the same list to avoid nested typewriter runs
    const leafTextEls = allTextEls.filter(el => {
      let p = el.parentElement;
      while (p && p.tagName !== 'BODY') {
        if (allTextEls.includes(p)) return false;
        p = p.parentElement;
      }
      return true;
    });
    ...
    leafTextEls.forEach(el => {
      // Safety guards: skip hero, nav, footer nav links, buttons, inputs, modals, scripts
      if (el.closest('#hero')) return;
      if (el.closest('nav')) return;
      if (el.closest('#global-modal')) return;
      if (el.tagName === 'BUTTON' || el.tagName === 'INPUT' || el.tagName === 'TEXTAREA') return;
      // Skip elements that are purely decorative numbers/symbols  
